...
# Route to the main index page (user interface)
@app.route("/")
def index():
    try:
...
# Admin functionality to add a document
...
# Admin functionality to delete a document
...
# Admin functionality to update a document
...
def answer():
    try:
        ...
        # Membuat daftar sumber dokumen dari metadata yang ditemukan
        source = list({f"{src.metadata['source']}" for src in source_doc})

        # Mengembalikan jawaban dan sumber dokumen
